    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in">
      <div className="bg-white dark:bg-zinc-900 text-zinc-900 dark:text-zinc-100 rounded-3xl shadow-2xl max-w-md w-full overflow-hidden border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="px-5 pt-4 flex gap-2 border-b border-stone-100 dark:border-stone-800">
          <button
            ...
            className={`pb-2.5 text-xs font-semibold border-b-2 transition-all cursor-pointer ${
              activeTab === 'qr'
                ? 'border-[#364FAB] text-[#364FAB] dark:text-[#E8ECFB]'
        ...
        <div className="p-5 space-y-4">
          ...
          {successMsg && (
            <div className="p-3 rounded-xl bg-[#E8ECFB] dark:bg-[#22336F]/40 text-[#364FAB] dark:text-[#E8ECFB] text-xs flex items-center gap-2">
          ...
          {activeTab === 'qr' && (
            <div className="text-center py-2 space-y-3">
              ...
              <div className="relative mx-auto w-56 h-56 bg-stone-950 rounded-2xl border-2 border-[#364FAB] flex flex-col items-center justify-center overflow-hidden shadow-inner">
                {/* Laser scan line */}
                <div className="absolute inset-x-0 h-0.5 bg-[#364FAB] animate-pulse top-1/2 -translate-y-1/2 shadow-[0_0_8px_#364FAB]"></div>

                <div className="w-36 h-36 border border-dashed border-[#E8ECFB]/60 rounded-xl flex flex-col items-center justify-center p-3 relative bg-black/40">
                  <QrCode className="w-16 h-16 text-[#E8ECFB]/60 animate-pulse" />
                ...
                <div className="grid grid-cols-4 gap-2 mt-2">
                  {tables.slice(0, 4).map((tbl) => (
                    <button
                      ...
                      className="p-2 rounded-xl bg-[#F7F8FA] dark:bg-stone-800 hover:bg-[#E8ECFB] dark:hover:bg-[#22336F]/40 text-[#202124] dark:text-stone-200 text-xs font-bold transition-all cursor-pointer border border-stone-200 dark:border-stone-700"
                    >
                      {tbl.tableNumber}
                    </button>
                  ))}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 px-3.5 py-2.5 rounded-xl border border-stone-300 dark:border-stone-700 bg-stone-50 dark:bg-stone-800 text-xs text-stone-900 dark:text-stone-100 uppercase font-mono tracking-wider focus:ring-2 focus:ring-[#364FAB] outline-none"
                  />
                  <button
                    onClick={handleReservationVerify}
                    className="px-4 py-2.5 rounded-xl bg-[#364FAB] hover:bg-[#2D428F] text-white text-xs font-bold cursor-pointer"
                  >
                    Check In
                  </button>
                </div>
              ...
              <div className="bg-stone-50 dark:bg-stone-800/50 p-3 rounded-xl border border-stone-200 dark:border-stone-700 text-xs space-y-1">
                ...
                {reservations.filter((r) => r.status === 'confirmed').map((r) => (
                  <div
                    ...
                    className="flex items-center justify-between p-2 rounded-lg bg-white dark:bg-stone-800 hover:bg-[#F3F5FD] dark:hover:bg-[#22336F]/40 cursor-pointer border border-stone-200/60 dark:border-stone-700"
                  >
                    <span className="font-mono font-bold text-[#364FAB] dark:text-[#E8ECFB]">{r.id}</span>
                ...
                {tables.map((t) => (
                  <button
                    ...
                    className={`p-3 rounded-xl border text-left transition-all cursor-pointer ${
                      t.status === 'occupied'
                        ? 'border-[#364FAB]/30 bg-[#F3F5FD]/50 dark:bg-[#22336F]/20'
                        ...
                        : 'border-stone-200 dark:border-stone-700 hover:border-[#364FAB] bg-white dark:bg-stone-800 hover:bg-[#F3F5FD]/50 dark:hover:bg-[#22336F]/30'
                    }`}
                  >
